    ...
    function combinedFilterWeeks(weeks) {
      ...
      function weekSorter(a, b) {
        ...
        if (
          ...
          const coachA = getCoachFromMembershipId(a.relatedMembership).user
            .name;
          const coachB = getCoachFromMembershipId(b.relatedMembership).user
            .name;
          if (coachA !== coachB) {
        ...
        <div>
          ...
          {contextual === `student${student.recordId}` && student.recordId && (
            <div className="studentPopup" ref={currentContextual}>
              ...
              {student.startingLevel.length > 1 && (
                <p>{student.startingLevel}</p>
              )}
        ...
        if (data.length === 0) {
          ...
          return data.map((call) => (
            <div className="assignmentBox" key={call.recordId}>
              <button
                type="button"
                onClick={() => openCallPopup(call.recordId)}
              >
                {call.rating}
              </button>
