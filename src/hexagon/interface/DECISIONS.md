...
- UI version is `useStudentUiVersion` (from the logged-in user's `studentRole` + `betaTester`) plus `UiScope` (`data-ui`), not a CSS `@if`. Chrome stays on the legacy `Nav` + sub header + paper canvas for logged-out visitors and every v1 session. `AppHeader` replaces that chrome only after myData confirms a beta-tester student. Loaders go through `LoadingScreen` so v1 never sits on a `PageShell`. Do not set `data-ui=v2` on `html`.
