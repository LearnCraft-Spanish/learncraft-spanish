...
- UI version is `useStudentUiVersion` (from the logged-in user's `studentRole` + `betaTester`) plus `UiScope` (`data-ui`), not a CSS `@if`. Do not set `data-ui=v2` on `html` until chrome is redesigned.
