...
```typescript
// ✅ Complete workflow orchestrating multiple units
export interface UseCustomAudioQuizResult {
  quizItems: QuizItem[];
  isLoading: boolean;
  error: Error | null;
  startQuiz: () => void;
...
export function useCustomAudioQuiz(): UseCustomAudioQuizResult {
...
export interface UseVerbCreationResult {
  ...
  createVerb: (data: VerbData) => Promise<void>;
}
...
```typescript
// ❌ Single-unit functionality (belongs in units/)
export function usePagination(items: Item[]) {
  const [page, setPage] = useState(1);
  return { page, setPage, paginated: items.slice(...) };
}

// ❌ Simple data fetching (belongs in queries/)
...
export function useCustomQuiz() {
  return httpClient.get('/api/quiz'); // NO!
}
...
// ❌ Direct unit usage from interface (must go through use-case)
// Interface should call useCustomQuiz(), not usePagination() directly
```
