...
export function useVocabularyAdapter() {
  return {
    getVocabulary: async () => {
      const data = await httpClient.get('/api/vocabulary');
      // ❌ NO transformation logic here - that belongs in units/useCases
      return data.filter(v => v.active);
    }
  };
}
