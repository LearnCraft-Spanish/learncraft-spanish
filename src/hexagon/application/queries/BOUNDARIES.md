...
export function useFlashcardsQuery(): UseFlashcardsQueryReturnType {
...
// ❌ Complex orchestration (belongs in useCases/)
export function useQuizDataQuery() {
  // ❌ NO! Orchestrating multiple queries - belongs in use-case
...
// ❌ Data transformation logic (belongs in units/)
export function useExampleQuery() {
  const { data } = useQuery({
    queryKey: ['examples'],
    queryFn: getExamples,
...
export function useVocabularyQuery() {
  // ❌ NO! Use adapters instead
  return useQuery({
    queryKey: ['vocabulary'],
    queryFn: () => httpClient.get('/api/vocabulary'),
  });
}
```
