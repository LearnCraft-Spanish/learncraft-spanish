...
export function AudioProvider({ children }: { children: ReactNode }) {
  // Provider setup (technical setup, not business logic)
...
export function useAudioContext(): AudioEngine {
  ...
  return context; // Just return context - NO business logic
}
```
...
```typescript
// ❌ Business logic in provider-accessing hook
export function useAudioContext(): AudioEngine {
  const context = use(AudioContext);
  if (!context) {
    throw new Error('useAudioContext must be used within AudioProvider');
  ...
  const currentTrack = getCurrentTrack(); // Business logic!
  return { ...context, isPlaying, currentTrack }; // NO!
}
