...
function PrivateCallInstance({
  ...
  student: MinimalCoachingStudent;
  ...
  return (
    <div className="cellWithContextual" key={call.callId}>
      ...
      >
        {call.callRating?.rating || 'No Call Rating'}
      </button>
...
export function PrivateCallView({
  ...
  // inputs & defaults
  const [rating, setRating] = useState(call.callRating?.rating || '');
  const [caller, setCaller] = useState(call.caller.email || '');
  const [date, setDate] = useState(
    new Date(call.callDate).toISOString().split('T')[0],
  );
  const [notes, setNotes] = useState(call.notes);
  const [areasOfDifficulty, setAreasOfDifficulty] = useState(
    call.areasOfDifficulty,
  );
  const [callType, setCallType] = useState(call.callType?.callType || '');
...
export default function ReadOnlyPrivateCallsCell({
  calls,
  // tableEditMode,
  student,
}: {
  ...
  // tableEditMode: boolean;
  student?: MinimalCoachingStudent | undefined | null;
}) {
