...
export function useSelectedLesson() {
  ...
  const { activeStudentQuery, activeProgram, activeLesson } = useActiveStudent() // Fetch the active student
  ...
  const allowedVocabulary = useMemo ((): string[] => {
    ...
    programsQueryData?.forEach((program) => {
      const foundLesson = program.lessons.find(item => item.recordId === selectionState.toLesson?.recordId)
      if (foundLesson) {
  ...
  const requiredVocabulary = useMemo ((): string[] => {
    ...
    programsQueryData?.forEach((program) => {
      const foundFromLesson = program.lessons.find(item => item.recordId === selectionState.fromLesson?.recordId)
      const foundToLesson = program.lessons.find(item => item.recordId === selectionState.toLesson?.recordId)
      if (foundFromLesson && foundToLesson) {
        ...
        const oldVocab = allFromVocab.filter(word => !newFromVocab.includes(word))
        requiredVocabulary = allToVocab.filter(word => !oldVocab.includes(word))
      }
    })
    return requiredVocabulary
  }, [programsQueryData, selectionState?.fromLesson?.recordId, selectionState?.toLesson?.recordId])

  // Function to filter flashcards by allowed vocabulary
  const filterExamplesBySelectedLesson = useCallback((examples: Flashcard[]): Flashcard[] => {
    const filteredByAllowedVocab = examples.filter((item) => {
      let isAllowed = item.vocabIncluded.length > 0 && item.vocabComplete !== false
  ...
  const setFromLesson = useCallback ((newId: number | string | null) => {
    ...
    const newFromLesson = selectionState.program?.lessons.find(item => item.recordId === newId)
    queryClient.setQueryData(['programSelection'], (oldState: typeof initialSelectionState) => ({
      ...oldState,
  ...
  // Set 'to' lesson
  const setToLesson = useCallback((toLesson: number | string | null) => {
    if (typeof (toLesson) === 'string') {
      toLesson = Number.parseInt(toLesson)
    }
    const newToLesson = selectionState.program?.lessons.find(item => item.recordId === toLesson)
    queryClient.setQueryData(['programSelection'], (oldState: typeof initialSelectionState) => ({
      ...oldState,
